...
export async function sourceScreen(root,ctx){
  ...
  if(!CONFIG.sources.length){root.append(empty('Source selection awaits the backend directory','This draft contract cannot discover your Source memberships. The backend operator must supply a documented membership endpoint or approved staging Source choices. No memberships have been inferred.'));return ()=>{};}
  const toolbar=el('div',null,{class:'toolbar'}),chooser=select('Your Source',[['','Select a Source'],...CONFIG.sources.map(s=>[s.id,s.name])],state.sourceId||'');
  ...
  const sourceId=state.sourceId;let profile,response;
  ...
  const p=profile.data,offers=response.data.items;
...
async function edit(offer,ctx,newBatch=false,resume=null){
  ...
  form.addEventListener('input',change);dates.addEventListener('input',change);food.input.addEventListener('change',()=>{if(!draft.lotId){unit.input.value=defaultUnit(foods.find(f=>f.id===food.input.value)?.name||'');change();}});
  if(offer&&!newBatch&&draft.baseRevision!==offer.revision){box.append(el('p','Conflict: the Source changed after this draft was saved. Publishing is blocked until you review the new stock and terms.',{class:'error'}),button('Review current server record',()=>modal('Current server state',el('pre',JSON.stringify(offer,null,2)))));}
  box.append(button('Save on this device',save),button('Review & publish',async()=>{
    ...
    const review=el('div');review.append(el('p',`${f.quantity} ${f.unit} ${draft.foodName} · ${priceText(f.price,f.unit)} · ${f.pressure}`),el('p','Confirm your physical count and collection window now. Any server revision conflict will preserve this draft.'),button('Confirm publication',async()=>{
      ...
      const result=await ctx.command('integrity_publish_v1',{source_id:source,lot_id:draft.lotId,expected_revision:draft.baseRevision,physical_quantity:f.quantity,price:f.price,lifecycle:f.lifecycle,pressure:f.pressure,visibility:f.visibility,reservable:f.reservable,listing_expires_at:f.listingExpiresAt,collection_start:f.collectionStart,collection_end:f.collectionEnd,note:f.note||null},source,{operationId:draft.publishId});

...
export function button(text,action,kind=''){const b=el('button',text,{type:'button',class:kind});b.addEventListener('click',async()=>{b.disabled=true;try{await action(b);}catch(e){message(e.message,true);}finally{b.disabled=false;}});return b;}
